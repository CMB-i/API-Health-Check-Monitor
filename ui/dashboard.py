...
def _status_cell(is_up: bool, error: str | None) -> Text:
    ...
    return Text(f"[ FAIL ]  DOWN", style="bold red")
...
def build_table(results: list[dict[str, Any]], cycle: int) -> Panel:
    """
    Build and return a ``rich.panel.Panel`` wrapping the health-check table.
    """
    # ── Table skeleton ──────────────────────────────────────────────────────
    table = Table(
        box=box.ROUNDED,
    ...
    # ── Surround with a panel for title + border ─────────────────────────────
    now_local = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d  %H:%M:%S")
    title = Text.assemble(
        (">> API Health Monitor", "bold bright_cyan"),
        ("  |  ", "dim"),
        (f"Cycle #{cycle}", "bold magenta"),
    )
    subtitle = Text(f"Last refresh: {now_local}", style="dim italic", justify="right")
...
def build_startup_panel() -> Panel:
    ...
    return Panel(msg, border_style="yellow", title="[bold]API Health Monitor[/bold]")
...
async def dashboard_loop(results_provider, interval=5):
    ...
    """
    cycle = 0
    with Live(build_startup_panel(), console=console, refresh_per_second=4) as live:
        while True:
            try:
